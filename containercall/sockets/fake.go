...
func (fake *Fake) Interpret(scope map[string]*model.Value, scgContainerCallSockets map[string]string, scratchDirPath string) (map[string]string, error) {
	...
	fake.recordInvocation("Interpret", []interface{}{scope, scgContainerCallSockets, scratchDirPath})
	fake.interpretMutex.Unlock()
	if fake.InterpretStub != nil {
		return fake.InterpretStub(scope, scgContainerCallSockets, scratchDirPath)
	}
	if specificReturn {
...
func (fake *Fake) InterpretArgsForCall(i int) (map[string]*model.Value, map[string]string, string) {
	fake.interpretMutex.RLock()
	defer fake.interpretMutex.RUnlock()
