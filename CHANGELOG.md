...
- Allow containers to register multiple names with opctl DNS (i.e. `container.dnsNames`)

### Deprecated

- `container.name`; use `container.dnsNames` (for registering names with opctl DNS) and/or `call.name` (for tracking in the call graph)
