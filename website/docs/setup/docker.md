...
The `opctl:0.1.32-dind` variant leverates Docker in Docker (DinD) and requires a `--privileged` flag.
...
docker run \
    --privileged \
    opctl/opctl:0.1.32-dind \
    opctl run github.com/opspec-pkgs/uuid.v4.generate#1.1.0
```

## dood variant

The `opctl:0.1.32-dood` variant leverages Docker out of Docker (DooD) and requires:
