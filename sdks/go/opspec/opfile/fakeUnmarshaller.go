...
type FakeUnmarshaller struct {
	UnmarshalStub        func(opFileBytes []byte) (*model.OpFile, error)
...
func (fake *FakeUnmarshaller) Unmarshal(opFileBytes []byte) (*model.OpFile, error) {
	...
	fake.recordInvocation("Unmarshal", []interface{}{opFileBytesCopy})
	fake.unmarshalMutex.Unlock()
	if fake.UnmarshalStub != nil {
		return fake.UnmarshalStub(opFileBytes)
	}
	if specificReturn {
...
func (fake *FakeUnmarshaller) UnmarshalArgsForCall(i int) []byte {
	fake.unmarshalMutex.RLock()
	defer fake.unmarshalMutex.RUnlock()
	return fake.unmarshalArgsForCall[i].opFileBytes
}
