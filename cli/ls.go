...
func ls(
	...
	for _, dirRef := range opPath {
		...
		for path, op := range opsByPath {
			...
			if filepath.IsAbs(opRef) && !strings.HasPrefix(opRef, nodeConfig.DataDir) {
				...
				opRef = strings.TrimPrefix(relOpRef, model.DotOpspecDirName+"/")
			} else {
				opRef = strings.TrimPrefix(path, "/")
			}
