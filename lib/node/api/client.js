...
class Client {
  ...
  /**
   * Gets op content at contentPath
   *
   ...
   * @return {Promise.<fetch.Response>}
   */
  op_content_get({opRef, contentPath}) {
    return fetch(
      `${this.baseHttpUrl}/ops/${encodeURIComponent(opRef)}/contents/${encodeURIComponent(contentPath)}`
    )
      .then(this._assertStatusSuccessful);
  }
   ...
   */
  op_content_list({opRef}) {
    return fetch(
      `${this.baseHttpUrl}/ops/${encodeURIComponent(opRef)}/contents`
    )
      .then(this._assertStatusSuccessful)
