...
class Client {
   ...
   * @param {Client~eventsStreamGet_onClose} onClose
   * @param {Client~eventsStreamGet_onError} onError
   * @param {Client~eventsStreamGet_onEvent} onEvent
   * @param {Client~eventsStreamGet_onOpen} onOpen
   *
  ...
  eventsStreamGet ({
    ...
    const webSocket = new WebSocketClient(`${this.baseWebsocketUrl}/events/stream?${queryParts.join('&')}`)
   ...
   * @param {Object} opStartReq.op
   * @param {String} opStartReq.op.ref
   * @param {Object} [opStartReq.op.pullCreds]
   * @param {String} [opStartReq.op.pullCreds.username]
   * @param {String} [opStartReq.op.pullCreds.password]
  ...
  dataGet ({dataRef}) {
    return fetch(
      `${this.baseHttpUrl}/data/${encodeURIComponent(dataRef)}`
    )
      .then(this._assertStatusSuccessful)
  }
}
