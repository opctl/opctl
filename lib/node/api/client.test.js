...
describe('op_content_get', () => {
  test('makes expected http request', async () => {
    /* arrange */
    const providedOpRef = '//dummyOpRef';
    const providedContentPath = '/dummyContentPath';

    const scope = nock(providedBaseUrl)
      .log(console.log)
      .get(
        `/ops/${encodeURIComponent(providedOpRef)}/contents/${encodeURIComponent(providedContentPath)}`)
      .reply('200');

    /* act */
    await objectUnderTest.op_content_get({
      opRef: providedOpRef,
  ...
  describe('response is 200', async () => {
    test('returns expected result', async () => {
      ...
      /* act */
      const actualContent = await objectUnderTest.op_content_get({
        opRef: providedOpRef,
        contentPath: providedContentPath
...
describe('op_content_list', () => {
  test('makes expected http request', async () => {
    ...
    const scope = nock(providedBaseUrl)
      .log(console.log)
      .get(
        `/ops/${encodeURIComponent(providedOpRef)}/contents`)
      .reply('200', []);

    /* act */
    await objectUnderTest.op_content_list({opRef: providedOpRef});
  ...
  describe('response is 300', async () => {
    test('returns expected result', async () => {
      ...
      let actualErr;
      try {
        await objectUnderTest.op_content_list({
          opRef: providedOpRef,
        });
      } catch (err) {
        actualErr = err;
  ...
  describe('response is 200', async () => {
    test('returns expected result', async () => {
      ...
      const expectedContentList = [{path: '/dummy/op/path'}];
      ...
      /* act */
      const actualContentList = await objectUnderTest.op_content_list({opRef: providedOpRef});
